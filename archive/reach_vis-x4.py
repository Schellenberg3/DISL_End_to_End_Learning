...
from custom import format_data
from custom import get_order
from custom import load_data
from custom import split_data_4
...
def get_model():
    ...
    z = Dense(128, activation="relu")(model_y.output)
    z = Reshape((1, 128))(z)
    ...
    _model = Model(inputs=model_y.input, outputs=z)
...
if live_demos:
    ...
    save_path = 'test_demos/reach_target/'
...
if no_model:
    ...
    *_image, *_label, and *_data all work like this 
    ...
    train_path = 'train_demos/reach_target/variation0/episodes'
    train_episodes = 200
    epochs = 10
    ...
    for train in train_order:
        ...
        train_data, train_images, train_label = split_data_4(demo)

        model.fit(x=np.asarray(train_images),
    ...
    model.save('saved_model/demo_1_testing_vis_only_4_deep_10e')

    print("\n\n---> Saved the model.\n\n")

else:
    model = tf.keras.models.load_model('saved_model/demo_1_testing_vis_only_4_deep_10e')
...
for test in test_order:
    ...
    demo = format_data(demo)
    test_data, test_images, test_label = split_data_4(demo)

    loss, acc = model.evaluate(x=np.asarray(test_images),
...
list_4_image = [blank_image, blank_image, blank_image, blank_image]
...
for i in range(evaluation_steps):
    ...
    list_4_image.pop(-1)  # remove last image
    image = np.dstack((obs.front_rgb, obs.front_depth))
    list_4_image.insert(0, image)

    vision_input = np.dstack((list_4_image[0], list_4_image[1], list_4_image[2], list_4_image[3]))

    # The model.predict() method requires a list, so even for one input we must expand the dimension by 1
    vision_input = np.expand_dims(vision_input, 0)

    action = model.predict(x=[vision_input])
